...
* **DEPRECATION** the list, range, sidebar, and tab bar widgets and themes are renamed to drop the redundant `Selector` prefix: `SelectorListTile` → `SelectListTile`, `SelectorListTileTheme` → `SelectListTileTheme`, `SelectorCheckboxListTile` → `SelectCheckboxListTile`, `SelectorRadioListTile` → `SelectRadioListTile`, `SelectorListView` → `SelectListView`, `SelectorListViewState` → `SelectListViewState`, `SelectorListSkeleton` → `SelectListSkeleton`, `SelectorRangeSlider` → `SelectRangeSlider`, `SelectorRangeSliderTheme` → `SelectRangeSliderTheme`, `SelectorRangeView` → `SelectRangeView`, `SelectorSideBar` → `SelectSideBar`, `SelectorSideBarTheme` → `SelectSideBarTheme`, `SelectorSideBarSkeleton` → `SelectSideBarSkeleton`, `SelectorTabBar` → `SelectTabBar`, `SelectorTabBarTheme` → `SelectTabBarTheme`, `SelectorTabBarIndicatorSize` → `SelectTabBarIndicatorSize`, and `SelectorTabBarSkeleton` → `SelectTabBarSkeleton`. The internal constant `kSelectorListTileHeight` was renamed to `kSelectListTileHeight`. The old names are retained as deprecated type aliases (e.g. `typedef SelectorListTile = SelectListTile;`) and a deprecated constant that behave identically and will be removed in a future minor version. Migrate call sites to the `Select*` names to clear the deprecation warnings.

* **DEPRECATION** the panel-content widget, theme, and related types are renamed to drop the redundant `Selector` prefix: `SelectorActionBar` → `SelectActionBar`, `SelectorActionBarTheme` → `SelectActionBarTheme`, `SelectorActionBarSkeleton` → `SelectActionBarSkeleton`, `SelectorActionBarBuilder` → `SelectActionBarBuilder`, `SelectorChipBar` → `SelectChipBar`, `SelectorChipBarTheme` → `SelectChipBarTheme`, `SelectorChipVariant` → `SelectChipVariant`, `SelectorExpansionTile` → `SelectExpansionTile`, `SelectorExpansionTileTheme` → `SelectExpansionTileTheme`, `SelectorFieldTile` → `SelectFieldTile`, `SelectorFieldTileTheme` → `SelectFieldTileTheme`, `SelectorFieldTileVariant` → `SelectFieldTileVariant`, `SelectorGridTile` → `SelectGridTile`, `SelectorGridTileTheme` → `SelectGridTileTheme`, `SelectorGridTileVariant` → `SelectGridTileVariant`, `SelectorGridView` → `SelectGridView`, `SelectorGridViewState` → `SelectGridViewState`, and `SelectorGridSkeleton` → `SelectGridSkeleton`. The old names are retained as deprecated type aliases (e.g. `typedef SelectorActionBar = SelectActionBar;`) that behave identically and will be removed in a future minor version. `SelectorActionBarVisibility` is package-internal (not re-exported from the public barrel) and is renamed cleanly to `SelectActionBarVisibility` without a deprecated alias. The internal constants `kSelectorChipBarHeight` / `kSelectorExpansionTileAnimationDuration` were renamed to `kSelectChipBarHeight` / `kSelectExpansionTileAnimationDuration`. Migrate call sites to the `Select*` names to clear the deprecation warnings.

* **DEPRECATION** the overlay and label types are renamed to a consistent `Select*` naming: `DropdownOverlay` → `SelectOverlay`, `DropdownOverlayStyle` → `SelectOverlayStyle`, `SelectorOverlayHost` → `SelectOverlayHost`, `SelectorLabelState` → `SelectLabelState`, and `SelectorLabelLoader` → `SelectLabelLoader`. The source files were renamed along the way (`lib/src/dropdown_overlay.dart` → `lib/src/select_overlay.dart`, `lib/src/dropdown_overlay_style.dart` → `lib/src/select_overlay_style.dart`, `lib/src/selector_overlay_host.dart` → `lib/src/select_overlay_host.dart`, and `lib/src/selector_label_state.dart` → `lib/src/select_label_state.dart`). The public types `DropdownOverlayStyle`, `SelectorLabelState`, and `SelectorLabelLoader` are retained as deprecated type aliases (e.g. `typedef DropdownOverlayStyle = SelectOverlayStyle;`) that behave identically and will be removed in a future minor version. `DropdownOverlay` and `SelectorOverlayHost` are package-internal (not re-exported from the public barrel) and are renamed cleanly without a deprecated alias. The internal constants `kDropdownOverlayMaxHeightFactor` / `kDropdownOverlayScreenMargin` were renamed to `kSelectOverlayMaxHeightFactor` / `kSelectOverlayScreenMargin`. Migrate call sites to the `Select*` names to clear the deprecation warnings.
